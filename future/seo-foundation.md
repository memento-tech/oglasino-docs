...
**Privacy:**
- `PrivacyPolicy` (existing, fix delivery; delete the broken inline `<script>`)

**Terms:**
- `TermsOfService` (existing, fix delivery)
...
### 6.4 Hreflang clusters are per-base-site
...
So a product page on `/rs-sr/product/123/...` emits hreflang siblings for `rs-sr`, `rs-en`, `rs-ru` only. Its catalog page emits hreflang siblings for those three. A motorcycle product on `/rsmoto-sr/product/456/...` emits siblings for `rsmoto-sr`, `rsmoto-en`, `rsmoto-ru` only. A `me` product emits four siblings: `me-sr`, `me-en`, `me-ru`, `me-cnr`.

The same applies to user profile URLs (users belong to one base-site), product detail URLs (products belong to one base-site), and catalog URLs (the catalog is base-site-scoped).

Static pages (about, pricing, free-zone, privacy, terms) are different: every base-site serves these pages in every available language, and the content really is a language variant of the same page. Static-page hreflang clusters include all ten routes.

Home and intro: same as static — all ten routes are hreflang siblings of `/` and of `/{locale}`.
...
- **Home** — add `<h1>` with a meaningful localized phrase. Suggested: the site tagline ("Oglasi Srbija — Kupi i prodaj" or per-locale equivalents). Translation key in `METADATA` namespace.
- **Catalog** — add `<h1>` with the bottom category name + region context if applicable ("Automobili u Srbiji"). Uses the existing category labelKey, translated via next-intl. If on a subcategory, `<h1>` is the bottom category; breadcrumbs (Section 8) carry the chain.
- **Product detail** — the second `<h1>` wrapping price (`ProductDetails.tsx:81`) becomes a `<p>` (or `<div>` if styling requires) with the existing visual treatment. The product name `<h1>` at `ProductDetails.tsx:69` stays.
- **About** — the SVG-only `<h1>` (`about/page.tsx:26`) gets text content. The visible heading at `:29` moves into the `<h1>`, and the SVG sits alongside or above.
- **User profile** — `UserDetails.tsx:118` displayName moves from `<div>` to `<h1>`.
- **Pricing** — `pricing/page.tsx:31`: change `hero.subtitle.line1` (twice) to `hero.title` + `hero.subtitle.line1` so the `<h1>` carries the page title and the subtitle is a `<p>` underneath. (Touches the existing 2026-05-14 `issues.md` entry — this fix closes it.)
- **Privacy / Terms** — H1 lives in the external markdown fetched from GitHub. Verify the markdown starts with a top-level heading. If not, add one in the page wrapper.
